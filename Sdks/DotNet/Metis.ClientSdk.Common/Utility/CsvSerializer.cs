...
namespace Metis.ClientSdk
{
    internal class CsvSerializer
    {
        ...
        /// </summary>
        public string SerializeSingle<T>(T obj) where T : LogEntity
        {
        ...
        /// </summary>
        public string SerializeMutil<T>(List<T> objList) where T : LogEntity
        {
            TypeAccessor ta = TypeAccessor.GetAccessor(typeof(T));
            var properties = ta.ReadWriteProperties;
        ...
        /// 将单个LogEntry对象序列化为CSV格式的字符串
        /// </summary>
        public string SerializeSingle<T>(Type realType, T obj) where T : LogEntity
        {
            //获取对象中可读写字段的Property信息
